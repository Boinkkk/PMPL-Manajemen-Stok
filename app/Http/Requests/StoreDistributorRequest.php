...
{
    /**
     * Tentukan apakah pengguna boleh menambah distributor.
     */
    ...
    /**
     * Aturan validasi tambah distributor.
     *
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                if ($namaDistributor !== '' && DB::table('distributor')->whereRaw('LOWER(nama_distributor) = ?', [mb_strtolower($namaDistributor)])->exists()) {
                    $validator->errors()->add('nama_distributor', 'Nama distributor sudah terdaftar.');
                }

                if ($email && DB::table('distributor')->whereRaw('LOWER(email) = ?', [mb_strtolower((string) $email)])->exists()) {
