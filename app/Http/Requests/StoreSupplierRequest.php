...
{
    /**
     * Tentukan apakah pengguna boleh menambah supplier.
     */
    ...
    /**
     * Aturan validasi tambah supplier.
     *
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                if ($namaSupplier !== '' && DB::table('supplier')->whereRaw('LOWER(nama_supplier) = ?', [mb_strtolower($namaSupplier)])->exists()) {
                    $validator->errors()->add('nama_supplier', 'Nama supplier sudah terdaftar.');
                }

                if ($email && DB::table('supplier')->whereRaw('LOWER(email) = ?', [mb_strtolower((string) $email)])->exists()) {
