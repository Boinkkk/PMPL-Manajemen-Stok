<x-layouts.app :title="'Detail Distributor '.$distributor->kode_distributor">
    <section class="flex flex-col gap-5" x-data="{ deleteModal: false, confirmation: '' }">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-2xl font-semibold">{{ $distributor->nama_distributor }}</h2>
                <p class="text-sm text-jamu-muted">{{ $distributor->kode_distributor }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('distributor.edit', $distributor) }}" class="rounded-md bg-jamu-secondary px-4 py-2 text-sm font-semibold text-jamu-primary-dark hover:bg-jamu-secondary-light">Edit</a>
                @if (auth()->user()?->isAdministrator())
                    @if ($hasTransactions)
                        <button type="button" disabled title="Distributor tidak dapat dihapus karena memiliki riwayat transaksi. Hubungi Administrator jika diperlukan." class="cursor-not-allowed rounded-md border border-red-100 px-4 py-2 text-sm text-red-300">Hapus</button>
                    @else
        ...
        @if ($hasTransactions)
            <div class="rounded-md border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-900">
                Distributor ini memiliki riwayat transaksi stok keluar sehingga tidak dapat dihapus.
            </div>
        @endif

        <div class="grid gap-4 rounded-md border border-jamu-border bg-jamu-surface p-4 md:grid-cols-4">
            <div>
                <p class="text-xs text-jamu-muted">Kode Distributor</p>
                <p class="font-medium">{{ $distributor->kode_distributor }}</p>
            </div>
            <div>
                <p class="text-xs text-jamu-muted">Telepon</p>
                <p class="font-medium">{{ $distributor->telepon ?: '-' }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="font-medium">{{ $distributor->kontak_person ?: '-' }}</p>
            </div>
            <div class="md:col-span-2">
                <p class="text-xs text-jamu-muted">Alamat</p>
                <p class="font-medium">{{ $distributor->alamat ?: '-' }}</p>
            </div>
            <div>
                <p class="text-xs text-jamu-muted">Tanggal Ditambahkan</p>
                <p class="font-medium">{{ $distributor->dibuat_pada_formatted }}</p>
            </div>
            <div>
        ...
        <div class="grid gap-3 md:grid-cols-5">
            ...
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
                <p class="text-sm text-jamu-muted">Total Unit Keluar</p>
                <p class="mt-1 text-2xl font-semibold">{{ number_format($stats['total_unit'], 0, ',', '.') }}</p>
            </div>
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
                <p class="text-sm text-jamu-muted">Total Nilai Distribusi</p>
            ...
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
                ...
                <p class="text-xs text-jamu-muted">{{ $service->tanggal($stats['transaksi_terakhir']?->tanggal_keluar ?? null) }}</p>
            </div>
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
        ...
        <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <h3 class="font-semibold">Riwayat Transaksi Stok Keluar</h3>
                    <p class="text-sm text-jamu-muted">Daftar pengiriman stok ke distributor ini.</p>
                </div>
                <form method="GET" action="{{ route('distributor.show', $distributor) }}" class="grid gap-2 md:grid-cols-4">
                    ...
                    <tbody class="divide-y divide-jamu-border">
                        @forelse ($transactions as $transaction)
                            <tr>
                                ...
                                <td class="px-4 py-3">{{ $service->tanggal($transaction->tanggal_keluar) }}</td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('stok-keluar.show', $transaction->id_stok_keluar) }}" class="rounded-md border border-jamu-border px-3 py-1.5 text-sm hover:bg-jamu-bg">Lihat Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center text-jamu-muted">Belum ada transaksi stok keluar untuk distributor ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
            <h3 class="font-semibold">Produk yang Pernah Didistribusikan</h3>
            <div class="mt-4 overflow-x-auto">
                <table class="min-w-full divide-y divide-jamu-border text-sm">
                    <thead class="bg-jamu-secondary-light/40 text-left text-xs uppercase tracking-wide text-jamu-muted">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right">Total Unit</th>
                            <th class="px-4 py-3">Terakhir Dikirim</th>
                    ...
                    <tbody class="divide-y divide-jamu-border">
                        @forelse ($products as $product)
                            <tr>
                                ...
                                <td class="px-4 py-3">{{ $service->tanggal($product->terakhir_dikirim) }}</td>
                                <td class="px-4 py-3 text-right">{{ number_format((int) $product->stok_terkini, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-jamu-muted">Belum ada produk yang pernah didistribusikan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-cloak x-show="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div @click.outside="deleteModal = false" class="w-full max-w-md rounded-md border border-jamu-border bg-jamu-surface p-5 shadow-xl">
                ...
                <p class="mt-2 text-sm text-jamu-muted">Ketik nama distributor untuk menghapus permanen data ini.</p>
                <form method="POST" action="{{ route('distributor.destroy', $distributor) }}" class="mt-4 flex flex-col gap-3">
                    @csrf
                    @method('DELETE')
                    <div class="rounded-md bg-jamu-bg px-3 py-2 text-sm font-semibold">{{ $distributor->nama_distributor }}</div>
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <button type="submit" :disabled="confirmation !== @js($distributor->nama_distributor)" class="rounded-md bg-red-600 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-red-200">Hapus</button>
                    </div>
                </form>
            </div>
        </div>
