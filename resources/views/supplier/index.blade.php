    ...
    <section class="flex flex-col gap-5" x-data="{ deleteModal: false, deleteAction: '', deleteName: '' }">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="text-sm text-jamu-muted">Kelola supplier dan pantau nilai pasokan stok masuk.</p>
            </div>
            <div class="flex flex-wrap gap-2">
                @if (auth()->user()?->isAdministrator())
                    <a href="{{ route('supplier.export', request()->query()) }}" class="rounded-md bg-jamu-green px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">Ekspor Excel</a>
                @endif
                <a href="{{ route('supplier.create') }}" class="rounded-md bg-jamu-secondary px-4 py-2 text-sm font-semibold text-jamu-primary-dark hover:bg-jamu-secondary-light">Tambah Supplier</a>
            </div>
        </div>

        <div class="grid gap-3 md:grid-cols-4">
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
                <p class="text-sm text-jamu-muted">Total Supplier</p>
                <p class="mt-1 text-2xl font-semibold">{{ number_format($summary['total_supplier'], 0, ',', '.') }}</p>
            </div>
            <div class="rounded-md border border-green-200 bg-green-50 p-4">
                <p class="text-sm text-green-800">Supplier Aktif</p>
                <p class="mt-1 text-2xl font-semibold text-green-900">{{ number_format($summary['supplier_aktif'], 0, ',', '.') }}</p>
            </div>
            <div class="rounded-md border border-yellow-200 bg-yellow-50 p-4">
                <p class="text-sm text-yellow-800">Supplier Tidak Aktif</p>
                <p class="mt-1 text-2xl font-semibold text-yellow-900">{{ number_format($summary['supplier_tidak_aktif'], 0, ',', '.') }}</p>
            </div>
            <div class="rounded-md border border-jamu-border bg-jamu-surface p-4">
                <p class="text-sm text-jamu-muted">Nilai Pasokan Bulan Ini</p>
                <p class="mt-1 text-2xl font-semibold">{{ $service->rupiah($summary['total_nilai_bulan_ini']) }}</p>
            </div>
        </div>

        <form method="GET" action="{{ route('supplier.index') }}" class="grid gap-3 rounded-md border border-jamu-border bg-jamu-surface p-4 md:grid-cols-[1fr_220px_auto_auto]">
            ...
            <select name="sort" class="rounded-md border-jamu-border bg-white px-3 py-2 text-sm">
                ...
                <option value="nilai_desc" @selected(($filters['sort'] ?? '') === 'nilai_desc')>Nilai pasokan terbesar</option>
            ...
            <a href="{{ route('supplier.index') }}" class="rounded-md border border-jamu-border px-4 py-2 text-center text-sm hover:bg-jamu-bg">Reset</a>
        </form>
        ...
        <x-table>
            <thead class="bg-jamu-secondary-light/40 text-left text-xs uppercase tracking-wide text-jamu-muted">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right">Nilai Pasokan</th>
            ...
            <tbody class="divide-y divide-jamu-border">
                @forelse ($suppliers as $supplier)
                    <tr class="hover:bg-jamu-bg">
                        ...
                        <td class="px-4 py-3">{{ $supplier->kontak_person ?: '-' }}</td>
                        <td class="px-4 py-3">{{ $supplier->telepon ?: '-' }}</td>
                        <td class="px-4 py-3">{{ $supplier->email ?: '-' }}</td>
                        <td class="px-4 py-3 text-right">{{ number_format((int) $supplier->total_transaksi, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right">{{ $service->rupiah($supplier->total_nilai_pasokan) }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('supplier.show', $supplier) }}" class="rounded-md border border-jamu-border px-3 py-1.5 text-sm hover:bg-jamu-bg">Detail</a>
                                <a href="{{ route('supplier.edit', $supplier) }}" class="rounded-md border border-jamu-border px-3 py-1.5 text-sm hover:bg-jamu-bg">Edit</a>
                                @if (auth()->user()?->isAdministrator())
                                    @if ((int) $supplier->total_transaksi > 0)
                                        <button type="button" disabled title="Supplier tidak dapat dihapus karena memiliki riwayat transaksi. Hubungi Administrator jika diperlukan." class="cursor-not-allowed rounded-md border border-red-100 px-3 py-1.5 text-sm text-red-300">Hapus</button>
                                    @else
                                        <button
                                            type="button"
                                            class="rounded-md border border-red-200 px-3 py-1.5 text-sm text-red-700 hover:bg-red-50"
                                            @click="deleteModal = true; deleteAction = '{{ route('supplier.destroy', $supplier) }}'; deleteName = '{{ addslashes($supplier->nama_supplier) }}'"
                                        >Hapus</button>
                                    @endif
                                @endif
                            </div>
        ...
        <div x-cloak x-show="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div @click.outside="deleteModal = false" class="w-full max-w-md rounded-md border border-jamu-border bg-jamu-surface p-5 shadow-xl">
                ...
                <p class="mt-2 text-sm text-jamu-muted">Ketik nama supplier untuk memastikan data yang dihapus benar.</p>
